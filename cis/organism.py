...
class Organism:
	...
	def __init__(self, parent_id, startx, starty, w, h, game_reference):
		...
		print(f'NEW {parent_id} {startx} {starty} {w} {h}', file=log)
	...
	def cycle(self):
		# OVERRIDE THIS FUNCTION AND SEND ANY INFO TO GAME WINDOW IN THE APP
		try:
		...
		if self._reproduction_cycle > common.max_iters_without_child or \
			self._errors > common.max_number_of_errors:
			self.die()
